...
type RoutineCardProps = {
  ...
  onSuggestHabit: (idx: number) => void;
  aiHabitSuggestions: string[];
  isLoading: boolean;
  isActive: boolean;
  onAddHabit: (idx: number, habit: string) => void;
};
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            onChange={(e) => onRate(index, Number(e.target.value))}
            className="w-16 border px-2 py-1 rounded text-sm"
          />
          <button onClick={() => onDelete(index)} className="text-red-500 text-sm">삭제</button>
        </div>
      </div>
